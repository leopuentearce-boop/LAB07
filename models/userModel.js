...
const User = {
    ...
    async getById(id) { const [rows] = await db.query(`SELECT ${PUBLIC_COLUMNS} FROM users WHERE id = ?`, [id]); 
    return rows[0] || null; 
    },

    async create({ name, email }) { const [result] = await db.query('INSERT INTO users (name, email) VALUES (?, ?)', [name, email]);   
    ...
    async delete(id) { const [result] = await db.query('DELETE FROM users WHERE id = ?', [id]); 
        return result.affectedRows > 0; 
    }, 
};
